      ...
      try {
        ...
        if (conference) {
          ...
          if (!participant) {
            ...
            // Unmute and take off hold (if they were in queue)
            await twilioClient
              .conferences(conferenceSidToUse)
              .participants(call.twilioCallSid)
              .update({
                muted: false,
                hold: false // Take off hold so they hear conference, not waitUrl
              });
            
            console.log(`✅ [TWILIO] Successfully unmuted participant and removed hold`);
          }
        } else {
  ...
  static async putOnHold(callId: string): Promise<void> {
    try {
      ...
      try {
        ...
        console.log(`✅ [TWILIO] Participant muted`);
      } catch (twilioError: any) {
      ...
      await prisma.call.update({
        ...
        data: {
          ...
          isOnHold: false // No hold state - just muted
        }
      });

      console.log(`✅ [PARTICIPANT] ${callId} is now muted (hold)`);
    } catch (error) {
